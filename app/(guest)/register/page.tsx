...
// ✅ FIX: Field now takes an `id` prop and wires it to the label via
// htmlFor. Every call site below passes id={fieldName} AND sets that same
// id on its <input>/<select>, so getByLabel() now works everywhere on this
// form — no more placeholder/position-based selector guessing needed.
function Field({
  id,
  label,
  required,
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={id} className="block text-xs font-bold text-[#131b2e]">
        {label}
      ...
      {error && (
        <p className="text-[10px] text-red-500 flex items-center gap-1" data-testid={`${id}-error`}>
...
export default function RegisterCommunityPage() {
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-[#faf8ff] flex items-center justify-center p-4" data-testid="register-success-screen">
        <div
          ...
          <div className="px-6 py-5">
            ...
            {submitError && (
              <div
                ...
                className="mb-5 flex items-start gap-2 bg-red-50 text-red-600 text-xs font-medium px-4 py-3 rounded-xl border border-red-200"
              >
            ...
            <form id="register-form" onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div className="grid grid-cols-[110px_1fr] sm:grid-cols-[140px_1fr] gap-3">
                <Field id="countryCode" label="Code" required>
                  <div className="relative">
                    <select
                      id="countryCode"
