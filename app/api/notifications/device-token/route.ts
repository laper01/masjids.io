...
import { proxyRequest, proxyPOST } from "@/lib/proxyHelper";
import type {
  MarkAllReadResponse,
...
export async function PATCH(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    // Use req.clone() so the original stream stays intact for proxyRequest.
...
/**
 * Route Handler — Notifications Infrastructure
 *
 * NOTIF-04  POST /api/notifications/device-token
 *   → Registers a push notification device token for the authenticated user.
 *     Accepts an FCM/APNS/web token + platform string.
 *     Actor is derived entirely from the Bearer JWT — no path param needed.
 *
 * Auth: Bearer JWT (required)
 *
 * Body: { token: string; platform: "fcm" | "apns" | "web" }
 */
...
/**
 * NOTIF-04 — Register a device token for push notifications
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    let body: RegisterDeviceTokenRequest;

    // FIX: use req.clone().json() so the original request stream is preserved
    // for proxyPOST → proxyRequest, which calls req.text() internally.
    // Without the clone, req.json() consumes the body and proxyRequest throws
    // "Body is unusable".
    try {
      body = await req.clone().json();
    } catch {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Validation failed.",
          errors: { token: ["token is required and must be a non-empty string."] },
        },
        { status: 422 }
      );
    }

    const VALID_PLATFORMS = ["fcm", "apns", "web"] as const;
      ...
      return NextResponse.json(
        {
          ...
          message: "Validation failed.",
          errors: {
            platform: [`platform must be one of: ${VALID_PLATFORMS.join(", ")}.`],
          },
        },
        { status: 422 }
      );
    ...
    // ── Proxy to backend (falls back to mock when backend is unavailable) ─────

    return proxyPOST<RegisterDeviceTokenResponse>(
      req,
      `/notifications/device-token`,
      buildMockResponse(platform as RegisterDeviceTokenRequest["platform"])
    );
  } catch (error) {
    console.error("[NOTIF-04] Unexpected error:", error);
    return NextResponse.json(
      { success: false, message: "Internal server error." },
      { status: 500 }
    );
